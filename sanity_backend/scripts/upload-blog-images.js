...
async function run() {
  console.log('----------------------------------------------------')
  console.log('Sanity Standard Post Desktop Image Uploader')
  ...
  // Fetch only posts categorized under "Blogs And Articles"
  console.log('Fetching standard posts in category "Blogs And Articles" from Sanity...')
  const posts = await client.fetch(`*[_type == "post" && "Blogs And Articles" in categories[]->title] { _id, title, "slug": slug.current, mainImage }`)
  console.log(`Found ${posts.length} standard posts in the category.`)
